...
//static int _clock_mode = -1; // 11/13/18 JH
...
static void _eth_phy_config_gpio(void)
{
	...
	//  // 11/13/18 JH
	// Reenable gpio. Was high, needs to stay high to keep reset enabled
	//if (_clock_mode != ETH_CLOCK_GPIO0_IN)
	//{
		//pinMode(0, OUTPUT);
	//}
...
bool ETHClass::begin(uint8_t phy_addr, int power, int mdc, int mdio, eth_phy_type_t type, eth_clock_mode_t clock_mode)
{
	...
	//_clock_mode = clock_mode;  // 11/13/18 JH
